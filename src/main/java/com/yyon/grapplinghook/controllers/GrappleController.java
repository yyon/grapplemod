...
//									vec pullxy = new vec(pull.x, 0, pull.z);
	...
	public boolean tryStepUp(Vec collisionmotion) {
		...
		Vec moveoffset = collisionmotion.changeLen(0.05D).add(0, entity.maxUpStep() + 0.01D, 0);
    ...
    public Vec checkRepel(Vec p, Level w) {
		...
		BlockPos pos = BlockPos.containing(p.toVec3d());
		if (hasBlock(pos, w)) {
			v.add_ip(0, 1, 0);
		} else {
    ...
    public Vec vecDist(Vec p, Vec v, Level w) {
    	for (double i = 0.5; i < 10; i += 0.5) {
			...
			Vec combined = p.add(v2);
    		BlockPos pos = BlockPos.containing(combined.toVec3d());

    		if (!hasBlock(pos, w)) continue;
